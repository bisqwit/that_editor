...
//"""""""0123456789ABCDEF0123456789ABCDEFG""""""""0123456789ABCDEF0123456789ABCDEFG""""""""
"??????????????????????????????????????????????????????????????????????????????????????????"
"??????????????????????????????????????????????????????????????????????????????????????????"
"?????????????????????????????????????????????????          ##   ##                ????????"
"????????         ##   ##                 ????????          #.######               ????????"
    ...
    signed int MarioOffset)
{
    const char* data =
      ...
      ?    (Mario21 + 6 + pose * unsigned(16+6)
          + y * unsigned(6*(NumMarioPoses+1)+16*NumMarioPoses)
          + MarioOffset)
      :    (Mario32 + 8 + pose * unsigned(33+8)
          + y * unsigned(8*(NumMarioPoses+1)+33*NumMarioPoses)
          + MarioOffset);
    ...
    unsigned width)
{
    ...
    */
    ...
    unsigned char RevisedFontData[ 5 * 32 ];

    const unsigned base = FatMode ? 0x80 : 0xC0;
    const unsigned short chartable[5] =
        { base + 0x3 + MarioColor,
          base + 0x4 + MarioColor,
          base + 0x8 + MarioColor,
          base + 0x9 + MarioColor,
    ...
    if(VidCellHeight >= 29) spritewide = 33;
